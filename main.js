...
function adicionaLista(memoriaLocal) {
  ...
  botao.innerText = "X";
  removeBotao(botao, memoriaLocal.id);
  lista.appendChild(li);
}
function removeBotao(botao, id) {
  botao.addEventListener("click", function () {
    const liBotao = this.parentNode;
    liBotao.remove();
    removeBotaoStorage(id);
  });
}

function removeBotaoStorage(id) {
  lembretes.splice(
    lembretes.findIndex((element) => element.id === id),
    1
  );
